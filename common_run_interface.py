...
def MyOpen(*args, **kwargs):
    ...
    ro_path = os.getenv('SUBPROCESS-RO', '/dev/shm/mg/gp')
    target = args[0]

    if (len(args) == 1) or (args[1] != 'w'):
        rw_file = args[0]

        if 'SubProcesses' in rw_file:
            if not os.path.exists(rw_file):
                _ = os.path.realpath(rw_file).split('/')
                target = os.path.join(ro_path, '/'.join(_[_.index('process'):]))
        print "OPENING %s" % target

    return __builtin__.open(target, *args[1:], **kwargs)
